...
This project is a webapp that helps users of the OliveTin app to write the YAML configuration file.

The OliveTin service that uses the configuration file can be found at `../OliveTin/service/` (relative to this workspace root).

The configuration file structure can be found at `../OliveTin/service/internal/config/config.go` (relative to this workspace root).

The Documentation for the OliveTin project can be found at https://docs.olivetin.app
...
Forms should be created like this;

<form>
    <label for = "forename">Forename
        <span class = "key-info">
            <code class = "key-name">forename</code>
            <a target = "_blank" class = "docs-button" title = "Example" href = "https://docs.olivetin.app/example.html">📖</a>
        </span>
        <span class = "description">Your first name.</span>
    </label>
    <input name = "forename" placeholder = "eg: James"></input>

    <label for = "surname">Surname
        <span class = "description">Your surname</span>
    </label>
    <input name = "surname" placeholder = "eg: Read"></input>
</form>

There are CSS rules for all form elements already. Do not add extra CSS for forms or form elements.
